...
from scene_update_panoptic_values import Scene
import json
...
if __name__ == "__main__": 
    ...
    for line in tqdm(f, total=num_lines):
        ...
        clusters_after_filter,num = scene.process_image()
        ## insert panoptic update related code
        panoptic_mask = np.load(os.path.join(seg, f"clip_{clip}/panoptic_inference/{frame}_panoptic.npy"))
        segmentation_mask = np.load(os.path.join(seg, f"clip_{clip}/semantic_inference/{frame}_semantic.npy"))
        with open(os.path.join(seg, f"clip_{clip}/panoptic_inference/{frame}_panoptic_segments.json"), 'r') as f:
            segments_info = json.load(f)

        new_segments_info = []
        does_ego_exist = False
        for segment in segments_info:
            if segment["category_id"] == 64:
                does_ego_exist = True
            if segment["category_id"] in [0, 1, 9, 14, 16 , 17, 18, 25, 26, 27, 28, 29, 31,35, 36, 37, 58, 62, 63]:
        ...
        for i in range(1,num+1):
            additional_segment = {"id": id_max + i, "isthing": False, "category_id": 65}
            new_segments_info.append(additional_segment)
        id_max += num
        if not does_ego_exist:
            additional_segment = {"id": id_max + 1, "isthing": False, "category_id": 64}
            new_segments_info.append(additional_segment)
            x,y = panoptic_mask.shape
            panoptic_mask[x-101:x-1, int(y/2)-100:int(y/2)+100] = id_max + 1
        
        id_max = np.max(panoptic_mask)

        ## declustering
        copy_at_present = new_segments_info
        for segment in copy_at_present:
            binary_mask = np.where(panoptic_mask == segment["id"], 1, 0)            
            # draw text on the largest component
            lw, num = measurements.label(binary_mask)
            unique, counts = np.unique(lw, return_counts=True)
            sorted_unique = [[x, y] for y, x in sorted(zip(counts, unique))]
            ## continous
            if segment["category_id"] in [2,3,4,5,6,7,8,10,11,13,15,23,24,30,32]:
                sorted_unique.reverse()
                skipped_biggest_blob = False
                for blob in sorted_unique:
                    if blob[0] == 0:
                        continue
                    else:

                        if skipped_biggest_blob:
                            # filter out small ones based on kind of continous object
                            if segment["category_id"] in [7, 8, 11, 13, 14 ]:
                                if blob[1] < 20000:
                                    panoptic_mask[lw == blob[0]] = 0
                                    continue
                            elif segment["category_id"] in [4, 6, 10, 15, 23, 24]:
                                if blob[1] < 10000:
                                    panoptic_mask[lw == blob[0]] = 0
                                    continue
                            else:
                            ...
                            # new big blog which is not biggest identified so split into new item
                            id_max += 1
                            panoptic_mask[lw == blob[0]] = id_max
                            additional_segment = {"id": id_max, "isthing": False, "category_id": segment["category_id"]}
                            # print(np.max(panoptic_mask))
                            new_segments_info.append(additional_segment)
                        else:
                            skipped_biggest_blob = True
            
            # distict objects
            else:
                if len(sorted_unique) > 1:
                    max_cluster = sorted_unique[-2][0]
                    # weird fix for bug caused due to next step
                    lw[lw == 0] = max_cluster
                    panoptic_mask[lw != max_cluster] = 0
            
            # print(sorted_unique_check)


        ##

        np.save(os.path.join(output_folder, f"clip_{clip}/{frame}_panoptic_updated.npy"), panoptic_mask)
        with open(os.path.join(output_folder, f"clip_{clip}/{frame}_panoptic_segments_updated.json"), "w") as outfile: 
